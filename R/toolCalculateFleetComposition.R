...
toolCalculateFleetComposition <- function(fleetESdemand, vehDepreciationFactors, vehSalesAndModeShares, annualMileage, loadFactor, baseYear, helpers) {
  ...
  for (i in timesteps[timesteps > baseYear]) {
    ...
    tmp[period == i, earlyRetirement := ifelse(demandNewAdditions < (0.1 * totalESdemand), TRUE, FALSE)]
  ...
  fleetVehNumbers <- fleetVehNumbers[, contribConstrYear := (contribConstrYear / (annualMileage * loadFactor)) * 10^3][, c("annualMileage", "loadFactor") := NULL]
