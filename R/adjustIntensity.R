...
#' @param intImproTab table that provides transportPolScen specific annual improvement rates, start year and end year
...
toolAdjustIntensity <- function(intensity, intImproTab, years){
  ...
  FV_techvar <- technology <- level <- start_year <- start_fade <- end_year <- end_fade <- annual_factor <- annual_improvement_rate <- factor <- EJ_Mpkm_final <-
    ...
    intensity_yrl[, c("subsectorL3", "subsectorL2", "subsectorL1", "sector") := NULL]
    intensity_yrl <- intensity_yrl[year >= 2020]
    intensity_yrl <- approx_dt(intensity_yrl,
                             xdata = seq(2020,years[length(years)]),
                             xcol = "year",
                             ycol = "EJ_Mpkm_final",
                             idxcols = c("region", "vehicleType", "technology", "sector_fuel"),
                             extrapolate = T)
    ...
    intensity_new <- merge(mimap, intensity_yrl, by = "vehicleType", all.y = TRUE)
    ...
    intensity_new <- intensity_new[year <= end_fade, c("region", "vehicleType", "technology", "sector_fuel", "year", "EJ_Mpkm_final")]
    ...
    intensity <- merge(intensity, intensity_new, by = c("region", "technology", "vehicleType", "year", "sector_fuel"), all = TRUE)
    intensity[, EJ_Mpkm_final := ifelse(is.na(EJ_Mpkm_final.y), EJ_Mpkm_final.x, EJ_Mpkm_final.y)]
    #If Baseline efficiency improvements outperform transportPolScen specific improvements after fade_out year (when the adjusted ones stay contstant), keep the Baseline
