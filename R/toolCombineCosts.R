...
toolCombineCosts <- function(mrtransportInput, annuity, fuelCosts, subsidies, decisionTree, yrs, filter){
  ...
  # Convert all cost components from US$2005/vehkm to US$2005/(p|t)km
    ...
    combinedCAPEXandOPEX[, unit := ifelse(univocalName %in% c(filter$trn_pass, "International Aviation"), "US$2005/pkm", "US$2005/tkm")]

  # add zeros for active modes (time value costs are treated seperately)
    nonMotorized <- unique(decisionTree[univocalName %in% c("Cycle", "Walk"), c("region", "univocalName", "technology")])
