...
toolApplyScenPrefTrends <- function(historicalPrefs, baselinePrefTrends, mitigationFactors, mitigationTechMap, yrs, GDPpcMER, GDPcutoff) {
  ...
  baselinePrefTrends <- melt(baselinePrefTrends, variable.name = "period", id.vars = setdiff(names(baselinePrefTrends), c("period", "value")))
  ...
  # combine calibrated historical preferences and baseline future trends
  ...
  PrefTrends <- merge(baselinePrefTrends, mitigationFactors, by = c("transportPolScen", "region", "level", "subsectorL1", "subsectorL2", "vehicleType", "technology"), all.x = TRUE)
  ...
  PrefTrends <- approx_dt(baselinePrefTrends, yrs, "period", "value", idxcols = setdiff(names(baselinePrefTrends), c("period", "value")), extrapolate = TRUE)
