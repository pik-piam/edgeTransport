...
toolLoadREMINDfuelPrices <- function(gdxPath, yrs){
 ...
 fuelPrices <- fuelPrices[, c("region", "period", "all_enty", "value")]
 ...
 fuelPrices[, value := value * tdptwyr2dpgj * GJtoMJ] # US$2005/MJ
 # map on EDGE-T structure
 fuelPrices <- merge(fuelPrices, mapEdgeToREMIND, by = "all_enty")[, all_enty := NULL]
 #Average over diesel and gasoline
 fuelPrices <- fuelPrices[, .(value = mean(value)), by = c("region", "period", "fuel")]
 ...
 if (length(unique(fuelPrices$region)) == 21) {
   decisionTree <- toolLoadDecisionTree("regionCode21")
 } else if (length(unique(fuelPrices$region)) == 12) {
 ...
 if(nrow(test)){
   ...
   fuelPrices[, value := ifelse(value <= 0.001, mean(value[value > 0.001]), value), by = c("period", "technology")]
 }
