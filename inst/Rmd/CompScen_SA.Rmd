...
#load historical data 
 ...
 ## ---- Read historical data ----
  ...
  ValComp_data <- rbind(data[variable %in% vars & region=="CHA" & period %in% c(2010,2020,2030,2050,2100)][,scenario:=NULL],Assu_data,ETP_RTS[period %in% c(2010,2020,2030,2050,2100)],MOT_data[period==2020])
...
for (scen0 in scen) {
  plotdata <- data_extended$Logit_Prices_S3S[scenario==scen0]
  ...
  p2 <- mipBarYearData(plotdata[period %in% y_bar & region %in% regionchoice],ylab="Prices logit_S3S [USD2005/pkm]")
...
if (Standalone_mode == TRUE){
  cat('\n## Prices S2S3\n')
  scen <- unique(data_extended$Logit_Prices_S2S3$scenario)
...
for (scen0 in scen) {
  ...
  p2 <- mipBarYearData(plotdata[period %in% y_bar & region %in% regionchoice],ylab="Prices logit_S2S3 [USD2005/pkm]")
  p2 <- p2 + theme_mip(size = 18)
  print(p2)}
...
if (Standalone_mode == TRUE){
  cat('\n## Prices S1S2\n')
  scen <- unique(data_extended$Logit_Prices_S1S2$scenario)
  
for (scen0 in scen) {
  plotdata <- data_extended$Logit_Prices_S1S2[scenario==scen0]
  plotdata <- plotdata[groupvalue %in% c("trn_pass_road_LDV_4W","trn_pass_road_LDV_2W") ]
  plotdata[,scenario:=NULL]
  setnames(plotdata, "groupvalue","scenario")
  cat(paste0("\n##",scen0,"\n"))
  
  p2 <- mipBarYearData(plotdata[period %in% y_bar & region %in% regionchoice],ylab="Prices logit_S1S2 [USD2005/pkm]")
  p2 <- p2 + theme_mip(size = 18)
  print(p2)}
...
if (Standalone_mode == TRUE){
  cat('\n## Prices VS1\n')
  scen <- unique(data_extended$Logit_Prices_VS1$scenario)
  
for (scen0 in scen) {
  plotdata <- data_extended$Logit_Prices_VS1[scenario==scen0]
  plotdata <- plotdata[groupvalue %in% c("Compact Car","Subcompact Car","Van","Midsize Car","Large Car","Mini Car","Large Car and SUV")  ]
  plotdata[,scenario:=NULL]
  setnames(plotdata, "groupvalue","scenario")
  cat(paste0("\n##",scen0,"\n"))
  
  p2 <- mipBarYearData(plotdata[period %in% y_bar & region %in% regionchoice],ylab="Prices logit_VS1 [USD2005/pkm]")
  p2 <- p2 + theme_mip(size = 18)
  print(p2)}
