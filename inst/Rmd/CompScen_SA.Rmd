...
listofruns: list.files("C:/Users/johannah/Documents/EDGE-Transport/Output","SSP2-Mix._2022-02-04.*", full.names = T)
...
```{r, fig.width=20, fig.height=15, eval = TRUE,echo=FALSE, results= 'asis'}
  vars <- c(
  ...
  for (reg0 in regionchoice){
    ...
    Shares <- data[variable %in% vars & region == reg0 & period %in% y_bar]
    Shares[,value:=round(value,1)]
    Shares <- dcast(Shares, variable + unit + region + scenario + model ~ period)
    Shares <- as.data.table(Shares)
    Shares <- Shares[match(vars,variable)]
    Shares <- addHtmlTableStyle(Shares,css.cell = rbind(rep("background: lightgrey; 
                                            font-size: 3
                                            em;", times = ncol(Shares)),
                                     matrix("",
                                            ncol = ncol(Shares),
                                            nrow = nrow(Shares))))
    t <- htmlTable(Shares,rnames=FALSE, caption = paste0("ES Shares trn_pass ",reg0))
    print(t)
    }
