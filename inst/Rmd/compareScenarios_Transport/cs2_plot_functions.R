...
#' Show Line Plots by Variable
#'
#' x-axis variable chosen by
#' user.
#'
#' Same as \code{\link{showLinePlots}} but with the variable specified by
#' \code{xVar} on x-axis. For every y-axis-value, we need a unique x-axis-value.
#' For historical data, there may be several sources / models of the same
#' variable. For the x-axis-variable a unique historical source / model is
#' chosen via \code{histRefModel}.
...
#' @return \code{NULL} is returned invisible.
#' @section Example Plots:
#' \if{html}{page 1: \figure{showLinePlotsByVariable1.png}{options: width="100\%"}}
#' \if{html}{page 2: \figure{showLinePlotsByVariable2.png}{options: width="100\%"}}
#' @examples
#' \dontrun{
...
#' vars <- c(
#'   "FE|Transport pCap")
#' showLinePlotsByVariable(data, vars, "GDP|PPP pCap")
#' }
#' @export
#' @importFrom rlang .data .env
#' @importFrom tidyr drop_na
