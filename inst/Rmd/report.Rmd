...
# p2 = plotinconv(inco_tech = pref$FV_final_pref, vt = "Midsize Car", reggroup = reggroup)
...
demandEJplotf = function(demandEJ, POP){
  ...
  pfreight=ggplot()+
    geom_area(data = demandEJ[year > 2010 & year <= maxyear  & aggr_mode %in% c("Freight")&region%in% EU_regions], aes(x=year, y=demand_EJ, group = interaction(vehicle_type_plot,aggr_mode), fill = vehicle_type_plot), color = "black", position= position_stack())+
    facet_wrap(~region, nrow = 4, scales = "free")+
  ...
  pcap1=ggplot()+
    geom_area(data = demandEJcap[year > 2020 & year <= maxyear  & region %in% c(EU_regions, NEU_regions)], aes(x=year, y=cap_dem, group = interaction(vehicle_type_plot,aggr_mode), fill = vehicle_type_plot), color = "black", position= position_stack())+
    facet_wrap(~region, nrow = 4, scales = "free")+
    ...
    pcap2=ggplot()+
    geom_area(data = demandEJcap[year > 2020 & year <= maxyear  & region %in% non_EU_regions], aes(x=year, y=cap_dem, group = interaction(vehicle_type_plot,aggr_mode), fill = vehicle_type_plot), color = "black", position= position_stack())+
    facet_wrap(~region, nrow = 4, scales = "free")+
    labs(x = "", y = "Final Energy demand [GJ/cap]")+
    theme_minimal()+
    scale_fill_manual(values = cols)+
...
demandEJLDVplotf <- function(demandEJ){
  ...
  p1 = ggplot()+
    geom_area(data = demandEJ[year > 2010 & year <= maxyear  & region %in% c(EU_regions, NEU_regions) & year <= maxyear], aes(x=year, y=demand_EJ, group = technology, fill = technology), color="black",position= position_stack())+
  ...
  p2 = ggplot()+
    geom_area(data = demandEJ[year > 2010 & year <= maxyear  & region %in% c(non_EU_regions, "World", "EUR", "NEU") & year <= maxyear], aes(x=year, y=demand_EJ, group = technology, fill = technology), color="black",position= position_stack())+
...
demandEJTruckplotf <- function(demandEJ){
  ...
  p1 = ggplot()+
    geom_area(data = demandEJ[year > 2010  & year <= maxyear & region %in% EU_regions & year <= maxyear], aes(x=year, y=demand_EJ, group = technology, fill = technology), color="black",position= position_stack())+
    ...
    p2 = ggplot()+
    geom_area(data = demandEJ[year > 2010 & year <= maxyear  & region %in% c(non_EU_regions, "EUR", "NEU", "World") & year <= maxyear], aes(x=year, y=demand_EJ, group = technology, fill = technology), color="black",position= position_stack())+
    labs(x = "", y = "Final energy demand for trucks [EJ]")+
    facet_wrap(~region, nrow = 4, scales = "free")+
    scale_fill_manual(values = cols)+
    theme_minimal()+
...
demandkmTruckplotf <- function(demandkm){
  ...
  p1 = ggplot()+
    geom_area(data = demandkm[year > 2010  & year <= maxyear & region %in% EU_regions & year <= maxyear], aes(x=year, y=demand_F, group = technology, fill = technology), color="black",position= position_stack())+
    labs(x = "", y = "Energy services demand for trucks [EJ]")+
    facet_wrap(~region, nrow = 4, scales = "free")+
    scale_fill_manual(values = cols)+
    theme_minimal()+
    theme(axis.text.x = element_text(angle = 90,  size = 7, vjust=0.5, hjust=1),
    ...
    p2 = ggplot()+
    geom_area(data = demandkm[year > 2010 & year <= maxyear  & region %in% c(non_EU_regions, "EUR", "NEU", "World") & year <= maxyear], aes(x=year, y=demand_F, group = technology, fill = technology), color="black",position= position_stack())+
    labs(x = "", y = " Energy services demand for trucks [EJ]")+
    facet_wrap(~region, nrow = 4, scales = "free")+
    scale_fill_manual(values = cols)+
    theme_minimal()+
...
demandEJ_IEAcomp <- function(demandEJ, IEA){
  ...
  p1 = ggplot()+
    geom_area(data = totdem[flow%in%c("ROAD", "ROAD LDV", "ROAD HDV") & region %in% EU_regions & year <= maxyear ], aes(x=year, y=value, group = interaction(model, flow), fill = flow), color="black",position= position_stack())+
    ...
    p5 = ggplot()+
    geom_bar(data = totdem1[year %in% c(2005, 2010, 2015) & flow %in% c("ROAD", "ROAD LDV", "ROAD HDV") & region %in% EU_regions], aes(x=model, y=value, group = interaction(model, flow), fill = flow), color="black", position="stack", stat = "identity", width = 0.5)+
    labs(x = "", y = "Final energy demand road [EJ]")+
...
demandkmplotf = function(demandkm, POP){
  ...
  ptot_freight = ggplot()+
    geom_area(data = demandkm[mode =="freight"& year > 2010 & year <= maxyear  & region %in% EU_regions], aes(x=year, y=demand_F, group = interaction(vehicle_type_plot,aggr_mode), fill = vehicle_type_plot), color="black",position= position_stack())+
    labs(x = "", y = "Energy Services demand [trillion pkm]")+
    facet_wrap(~region, nrow = 4, scales = "free")+
    theme_minimal()+
    scale_fill_manual(values = cols)+
  ...
  pcap_freight1 = ggplot()+
    geom_area(data = demandkmcap[mode == "freight" & year >= 2020  & year <= maxyear & region %in% c(EU_regions, NEU_regions)], aes(x=year, y=cap_dem/1000, group = vehicle_type_plot, fill = vehicle_type_plot), color="black",position= position_stack())+
    ...
    geom_area(data = demandkmcap[mode == "freight" & year >= 2020  & year <= maxyear & region %in% c(non_EU_regions)], aes(x=year, y=cap_dem/1000, group = vehicle_type_plot, fill = vehicle_type_plot), color="black",position= position_stack())+
    labs(x = "", y = "Energy Services demand [ktkm/cap]")+
    theme_minimal()+
    facet_wrap(~region, nrow = 4, scales = "free")+
    scale_fill_manual("Vehicle Type", values = cols)+
  ...
  pcap_pass = ggplot()+
    geom_area(data = demandkmcap[mode == "pass" & year >= 2020 & year <= maxyear & region %in% c(EU_regions)], aes(x=year, y=cap_dem/1000, group = vehicle_type_plot, fill = vehicle_type_plot), color="black",position= position_stack())+
  ...
  pcap_pass2 = ggplot()+
    geom_area(data = demandkmcap[mode == "pass" & year >= 2020 & year <= maxyear & region %in% c(non_EU_regions)], aes(x=year, y=cap_dem/1000, group = vehicle_type_plot, fill = vehicle_type_plot), color="black",position= position_stack())+
    labs(x = "", y = "Energy Services demand [kpkm/cap]")+
    theme_minimal()+
    facet_wrap(~region, nrow = 4, scales = "free")+
    scale_fill_manual("Vehicle Type",values = cols)+
