...
for (reg0 in CostReg) {
  ...
  p2 <- mipBarYearData(plotdata, ylab = "Prices logit S1S [USD2005/pkm]", title = paste0("Prices logit S1S ",reg0) )
...
for (reg0 in CostReg) {
  ...
  p2 <- mipBarYearData(plotdata, ylab = "Prices logit S2 [USD2005/pkm]", title = paste0("Prices logit S2S1 ",reg0))
  p2 <- p2 + theme_mip(size = 18)
  print(p2)
  cat("\n\n")}
...
for (reg0 in CostReg) {
  ...
  p2 <- mipBarYearData(plotdata, ylab = "Prices logit S3S2 [USD2005/pkm]", title = paste0("Prices logit S3S2 ",reg0))
  p2 <- p2 + theme_mip(size = 18)
  print(p2)
  cat("\n\n")}
